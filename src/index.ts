...
interface AppDetailsData {
    [x: string]: {
        success: boolean;
        // TODO: Double check that {} is a good return type
        data: {};
    };
}
...
export default class SteamWrapper {
    ...
    // TODO: Double check that {} is a good return type
    public static async GetAppDetails(appid: string, filters: string[] = []): Promise<{}> {
        const {
            data: {
                [appid]: {
                    success,
    ...
    public async GetPlayerSummaries(...steamids: string[]): Promise<{ [steamid: string]: undefined | false | PlayerSummary }> {
        const {
        ...
        const defaults: {[x: string]: false | undefined } = zipObject(steamids, Array(steamids.length).fill(false));
